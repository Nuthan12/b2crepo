...
<c:choose>
		<c:when test="${fn:toUpperCase(currentLanguage.getIsocode()) eq 'ZH'}">
			...
			<c:if test="${not empty deliveryAddress.city && not empty deliveryAddress.city.name}">
				,&nbsp;${fn:escapeXml(deliveryAddress.city.name)}
			</c:if>
			<c:if test="${empty deliveryAddress.city || empty deliveryAddress.city.name}">
				,&nbsp;${fn:escapeXml(deliveryAddress.town)}
			</c:if>
			<c:if test="${not empty deliveryAddress.cityDistrict && not empty deliveryAddress.cityDistrict.name}">
				,&nbsp;${fn:escapeXml(deliveryAddress.cityDistrict.name)}
			</c:if>
